...
#define tst_tags__(n_, _1, _2, _3, _4, _5, _6, _7, _8)                                                                   \
  static unsigned char tst_tag_##_1 = 0x01;                                                                              \
  static unsigned char tst_tag_##_2 = 0x02;                                                                              \
  static unsigned char tst_tag_##_3 = 0x04;                                                                              \
  static unsigned char tst_tag_##_4 = 0x08;                                                                              \
  static unsigned char tst_tag_##_5 = 0x10;                                                                              \
  static unsigned char tst_tag_##_6 = 0x20;                                                                              \
  static unsigned char tst_tag_##_7 = 0x40;                                                                              \
  static unsigned char tst_tag_##_8 = 0x80;                                                                              \
...
#define TST_STR_HELP_TAGS " [+/-]tag ... ]\ntags:"
  ...
  static inline short tst_parse_tags(int argc, const char **argv, int ntags, const char **names)
  {
    ...
    for (int n = 0; n < argc; n++)
    {
      arg = argv[n];
      while (*arg)
      { // Allow a single string to contain multiple options (e.g. "--color +NoDB")
        v = 0xFF;
        if ((arg[0] == '-') && (arg[1] == '-'))
        {
          switch (arg[2])
          ...
          case 'h':
            ...
            if (ntags > 0)
              fputs(TST_STR_HELP_TAGS, stderr);
            goto prttags;
          case 'l':
            fprintf(stderr, "%s \"%s\"", argv[0], tst_title);
          prttags:
        ...
        for (int k = 0; k < ntags; k++)
        {
          ...
          {
            if (v)
              tst_tags_val |= (unsigned char)(1 << k);
            else
              tst_tags_val &= (unsigned char)(~(1 << k));
          }
        }
...
#define tstrun_(tst_, title_, ...)                                                                                                                                                      \
  ...
  {                                                                                                                                                                                     \
    ...
    if (CLOCKS_PER_SEC > ((clock_t)1000000) + tst_zero)                                                                                                                                 \
      tst_clock_unit = "n";                                                                                                                                                             \
    else if (CLOCKS_PER_SEC > ((clock_t)1000) + tst_zero)                                                                                                                               \
      tst_clock_unit = "u";                                                                                                                                                             \
    else                                                                                                                                                                                \
      tst_clock_unit = "m";                                                                                                                                                             \
    fprintf(stderr, "----- %s%s %s \"%s\" %s%s%s\n", tst_color + tst_str_cyan, tst_str_file, __FILE__, tst_title, tst_time(), tst_color + tst_str_normal, (tst_ ? "" : " (disabled)")); \
    if (tst_)                                                                                                                                                                           \
      tst__run(tst_usestatic);                                                                                                                                                          \
    fputs(tst_str_file_end, stderr);                                                                                                                                                    \
    tst_prt_results(tst_fail, tst_pass, tst_skip);                                                                                                                                      \
    fprintf(stderr, " %s\n", tst_time());                                                                                                                                               \
...
#define tstcheck_(tst_abrt, tst_str, tst_res, ...)                \
  ...
  {                                                               \
    ...
    case -1:                                                      \
      tst_skip++;                                                 \
      tst_case_skip++;                                            \
      tst_prtln(tst_str_skip);                                    \
      fputs(tst_color + tst_str_yellow, stderr);                  \
      break;                                                      \
    case 0:                                                       \
      tst_fail++;                                                 \
      tst_case_fail++;                                            \
      tst_prtln(tst_str_fail);                                    \
      fputs(tst_color + tst_str_red, stderr);                     \
      break;                                                      \
    case 1:                                                       \
      tst_pass++;                                                 \
      tst_case_pass++;                                            \
      tst_prtln(tst_str_pass);                                    \
      fputs(tst_color + tst_str_green, stderr);                   \
      break;                                                      \
    }                                                             \
...
#define tstskipif(tst_)                                                                                          \
  for (int tst_k = 1;                                                                                            \
    ...
    if (tst_skip_test && (tst_prtln(""), tst_prtf(tst_str_skip_tst, #tst_)))                                     \
      ;                                                                                                          \
    else
...
#define tstsection(...)                                                                                                                                                                                       \
  for (int tst_sect = 1;                                                                                                                                                                                      \
       tst_sect && ((tst_sect_counter > tst_sect_not_last) || !(tst_sect_counter = tst_sect_not_last)) && (++tst_sect_counter == tst_sect_iterator) && !(tst_prtln(tst_str_sctn), tst_prtf(" " __VA_ARGS__)); \
